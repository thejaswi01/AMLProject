...
% SVM linear
tic;
svm_mdl = fitclinear(train_images, train_labels);
end_time = toc;
%save('./svm_mdl', svm_mdl);
predict_labels = predict(svm_mdl, test_images);

svm_linear_acc = sum(predict_labels == test_labels) / size(test_labels, 1) 
fprintf('\n SVM linear Accuracy: %f ', svm_linear_acc * 100);
fprintf('\n SVM linear Error: %f ', (1 - svm_linear_acc) * 100);
fprintf('\n SVM linear Training time: %f ', end_time);